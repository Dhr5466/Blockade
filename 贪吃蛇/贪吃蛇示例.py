...
# 初始化蛇
def init_snake():
    snake = deque()#双向队列（deque）类似于list的容器，可以快速的在队列头部和尾部添加、删除元素
    snake.append((2, Area_y[0]))#左边是x轴，右边y轴，从左上角开始  Area_y[0]代表y轴最上端
...
def main():
    pygame.init()
    screen = pygame.display.set_mode((Screen_Width, Screen_Height),flags = 0 )  # 初始化一个准备显示的窗口或屏幕
    #   flag位置的值
    #   0 用户设置的窗口大小
    #   pygame.FULLSCREEN 创建一个全屏窗口
    pygame.display.set_caption('贪吃蛇')  # 左上角游戏名字
    # 得分字体设置
    #print("获取系统中所有可用字体", pygame.font.get_fonts())
    font1 = pygame.font.SysFont('SimHei', 24)#直接从系统中加载字体使用
    # GO字体设置
    font2 = pygame.font.SysFont(None, 72)
    fwidth, fheight = font2.size('GAME OVER')  ###该函数返回渲染文本所需的尺寸大小，返回值是一个一元组 (width,height)
